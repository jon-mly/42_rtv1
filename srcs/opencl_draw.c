...
void			opencl_draw(t_opencl *opcl, t_env *e)
{
	...
	if (opcl->err != CL_SUCCESS)
		printf("Error enqueuing objects %d", opcl->err);
	else
	...
	opcl->err = clEnqueueWriteBuffer(opcl->commands, opcl->structlight, CL_TRUE,
			...
			NULL, NULL);
	if (opcl->err != CL_SUCCESS)
		printf("Error enqueuing lights %d", opcl->err);
	else
	...
	if (opcl->err != CL_SUCCESS)
		printf("Error enqueuing camera %d", opcl->err);
	else
	...
	if (opcl->err != CL_SUCCESS)
		printf("Error enqueuing scene %d", opcl->err);
	else
	...
	if (opcl->err != CL_SUCCESS)
		printf("Error enqueuing xy array %d", opcl->err);
	else
	...
	if (opcl->err != CL_SUCCESS)
		printf("Error enqueuing output %d", opcl->err);
	else
